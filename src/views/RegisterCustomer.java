        ...
        if (e.getSource() == submitBtn) {
            ...
            if (nameTemp.isEmpty() || phoneNoTemp.isEmpty() || addressTemp.isEmpty() || emailTemp.isEmpty()
                ...
                try {
                    ...
                    rs = Database.getInstance().executeQuery("select cast(right(CustomerId, 3) as unsigned) + 1 as `NextCustomerId`\n" +
                            "from Customer\n" +
                            "order by CustomerId desc\n" +
                            "limit 1");
                    if (rs.next()) {
                        nextCustomerId = rs.getInt("NextCustomerId");
                    }
                    ...
                    Database.getInstance().executeUpdate(
                            "INSERT INTO `customer`(`UserId`, `CustomerId`) VALUES ('" + UserId + "', '" + String.format("CS%03d", nextCustomerId) + "')");
